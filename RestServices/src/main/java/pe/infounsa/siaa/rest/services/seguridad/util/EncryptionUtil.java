...
public class EncryptionUtil {
	...
	private Cipher getEncryptionCipher(final String value)
		...
		final SecretKey key = SecretKeyFactory.getInstance(
				DES_ENCRYPTION_ALGORITHM).generateSecret(keySpec);
	...
	private Cipher getDecryptionCipher(final String value)
		...
		SecretKey key = SecretKeyFactory.getInstance(DES_ENCRYPTION_ALGORITHM)
	...
	public String decrypt(final String encryptedValue, final String password)
			throws IOException, GeneralSecurityException {
		if (this.encryptionEnabled) {
			byte[] decodedValue = new sun.misc.BASE64Decoder()
					.decodeBuffer(encryptedValue);
			byte[] unencryptedBytes = this.getDecryptionCipher(password)
	...
	public String encrypt(final String plainValue, final String password)
			throws IOException, GeneralSecurityException {
		if (this.encryptionEnabled) {
			final byte[] encryptedBytes = this.getEncryptionCipher(password)
					.doFinal(plainValue.getBytes(ENCODING_CHARSET));
			final String base64WithNewlines = new sun.misc.BASE64Encoder()
					.encode(encryptedBytes);
			return base64WithNewlines.replace(
